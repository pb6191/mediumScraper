...
for i, h in enumerate(headlineEles, start=1):
    try:
        desc = h.find_element(By.XPATH, "../following-sibling::div/h3/a")
        desctext = desc.text
    except:
        desctext = ""
    writerSource = h.find_elements(By.XPATH, "../../../following-sibling::div/div/div/div/a")
    try:
        writer = writerSource[0]
        writertext = writer.text
    except:
        writertext = ""
    try:
        source = writerSource[1]
        sourcetext = source.text
    except:
        sourcetext = ""
    try:
        date = h.find_element(By.XPATH, "../../../following-sibling::div/div/div/div/div/div")
        datetext = date.text
    except:
        datetext = ""
    print(desc.text)
    ...
    write_csv(
        header=["headline", "desc", "writer", "where", "date"],
        data=zip([h.text], [desctext], [writertext], [sourcetext], [datetext]),
